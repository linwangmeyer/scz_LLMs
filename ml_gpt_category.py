...
# load data from Tori
df_gpt = pd.read_csv(parent_folder+'slopes_with_controls_forLin.csv')
...
# group effect
r, p_value = pearsonr(df_ml['PatientCat'], df_ml['slopes_with_control_vars'])
print(f'correlation between Patient category and slopes_with_control_vars:'
      f'\ncorrelation {r},'
      f'\np value: {p_value}')

# visualize correlation matrix
df_roi = df_ml[['entropyApproximate','slopes_with_control_vars','PatientCat','n_1','n_2','n_3','n_4','n_5']]
...
sns.scatterplot(data=df_roi, x='PatientCat', y='slopes_with_control_vars')
slope, intercept = np.polyfit(df_roi['PatientCat'], df_roi['slopes_with_control_vars'], 1)
...
plt.figure()
df_roi.boxplot(column='slopes_with_control_vars', by='PatientCat')
plt.show()
...
df_X = df_ml['slopes_with_control_vars']
