...
# group effect
r, p_value = pearsonr(df_avg['PatientCat'], df_avg['entropyApproximate'])
print(f'correlation between Patient category and entropyApproximate:'
      f'\ncorrelation {r},'
      f'\np value: {p_value}')

# visualize correlation matrix
df_roi = df_avg[['entropyApproximate','PatientCat','n_1','n_2','n_3','n_4','n_5']]
...
df_avg = filtered_df.groupby('ID')[['PatientCat','n_sentence','entropyApproximate','n_1','n_2','n_3','n_4','n_5']].mean().reset_index()
df_avg['PatientCat'] = df_avg['PatientCat'].map({1.0: 0, 2.0: 1})

df_X = df_avg[['entropyApproximate','n_1','n_2','n_3']]
df_y = df_avg['PatientCat']
...
# Use cross-validation with stratified k-fold (k=5 in this example)
cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=123)

# Perform cross-validation and get accuracy scores
cv_scores = cross_val_score(logreg, X_scaled, df_y, cv=cv, scoring='accuracy')
...
plt.savefig('roc_curve_plot.eps', format='eps', bbox_inches='tight')
plt.show()
